...
func init() { // 主函数
	...
	en.OnCommandGroup([]string{"服务详情", "service_detail"}, zero.UserOrGrpAdmin).SetBlock(true).Handle(func(ctx *zero.Ctx) {
		manager := ctx.State["manager"].(*ctrl.Control[*zero.Ctx]).Manager
		if manager == nil {
			ctx.SendChain(message.Text("ERROR: manager not found"))
			return
		}
		lenmap := len(manager.M)
		i, j := 0, 0
...
func extendCanvas(one *gg.Context, newH int) *gg.Context {
	if newH <= one.H() {
		return one
	}
	imgtmp := gg.NewContext(one.W(), newH)
	imgtmp.SetRGB255(255, 255, 255)
	imgtmp.Clear()
	imgtmp.DrawImage(one.Image(), 0, 0)
	return gg.NewContextForImage(imgtmp.Image())
}

func createDoublePic(mp *mpic, one *gg.Context, lt *location) (*gg.Context, error) {
	...
	imRY, imLY := imgs[0].Bounds().Dy()+100, imgs[1].Bounds().Dy()+5
	if mp.isDisplay {
		imLY += mp.h + 5
	}
	max := imRY
	...
	one = extendCanvas(one, max+int(mp.fontSize))
	if mp.isDisplay {
		one = extendCanvas(one, imLY)
		one.DrawImage(mp.im, 0, 0)
		one.DrawImage(imgs[1], 0, mp.h+5)
		one.DrawImage(imgs[0], 1280, 0)
	} else {
		one.DrawImage(imgs[0], 0, 0)
		one.DrawImage(imgs[1], 1280, 0)
	}
	return one, nil
}

func createSinglePic(mp *mpic, one *gg.Context, lt *location) (*gg.Context, error) {
	titlec := titleColor{isRandom: true}
	titlec.randfill()
	img, err := mp.createPic2(lt, &titlec, mp.info)
	if err != nil {
		return nil, err
	}
	imY := img.Bounds().Dy()
	one = extendCanvas(one, imY+int(mp.fontSize))
	if mp.isDisplay {
		one = extendCanvas(one, mp.h)
		if mp.isCustom {
			one.DrawImage(mp.im, 0, (one.H()-mp.h)/2)
		} else {
			one.DrawImage(mp.im, 0, 0)
		}
		one.DrawImage(img, one.W()-1280, 50)
	} else {
		one.DrawImage(img, 0, 50)
	}
	return one, nil
}

func (mp *mpic) createPic(one *gg.Context, lt *location) (image.Image, error) {
	var result *gg.Context
	var err error
	if mp.isDouble {
		result, err = createDoublePic(mp, one, lt)
	} else {
		result, err = createSinglePic(mp, one, lt)
	}
	if err != nil {
		return nil, err
	}
...
func (mp *mpic) downloadFont(fontPath string) error {
	if file.IsExist(fontPath) {
		return nil
	}
	url := "https://gitcode.net/u011570312/zbpdata/-/raw/main/" + fontPath[5:] + "?inline=true"
	return downloadData(fontPath, url)
}

func (mp *mpic) loadpic() error {
	if err := mp.downloadFont(mp.font1); err != nil {
		return fmt.Errorf("加载字体1失败: %w", err)
	}
	if err := mp.downloadFont(mp.font2); err != nil {
		return fmt.Errorf("加载字体2失败: %w", err)
	}
	if !mp.isDisplay {
		return nil
	}
	...
	if err != nil {
		return fmt.Errorf("打开看板娘图片失败: %w", err)
	}
	defer f.Close()
	mp.im, _, err = image.Decode(f)
	if err != nil {
		return fmt.Errorf("解码看板娘图片失败: %w", err)
	}
...
func downloadData(path, url string) error {
	...
	if len(data) == 0 {
		return errors.New("下载数据为空")
	}
	logrus.Printf("[draw]从镜像下载数据 %d 字节...", len(data))
	if err := os.WriteFile(path, data, 0644); err != nil {
		return fmt.Errorf("写入文件 %s 失败: %w", path, err)
	}
	logrus.Printf("[draw]下载文件成功: %s", path)
	return nil
}
