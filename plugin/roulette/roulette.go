...
func init() { // 插件主体
	...
	dataPath := engine.DataFolder() + "roulette.json"
	checkFile(dataPath)
	...
	checkSession := func(ctx *zero.Ctx) bool {
		...
		default:
			if ss == nil || ss.GroupID != ctx.Event.GroupID {
				return false
			}
			if ss.IsValid {
				if ss.isExpire() {
	...
	engine.OnFullMatch("开始轮盘赌", zero.OnlyGroup, checkSession).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			...
			ss := getSession(gid, dataPath)
			if ss == nil {
				ctx.SendChain(message.ReplyWithMessage(ctx.Event.MessageID, message.Text("游戏会话不存在, 请先创建游戏"))...)
				return
			}
			if !ss.checkJoin(uid) {
			...
			stop, cancelStop := zero.NewFutureEvent("message", 8, true,
				...
				Repeat()
			defer cancelStop()
			next := zero.NewFutureEvent("message", 500, false,
				zero.FullMatchRule("开火"),
				zero.OnlyGroup,
				func(eventCtx *zero.Ctx) bool {
					return eventCtx.Event.GroupID == gid
				})
			...
			for {
				select {
				case <-tick.C:
					ctx.SendChain(message.Text("轮盘赌还有15s过期"))
				case <-after.C:
					ss.close(dataPath)
					ctx.Send(
...
func getTruthOrDare(ctx *zero.Ctx) {
	...
	if rand.Intn(2) == 0 {
		ctx.SendChain(message.Text("真心话: ", truths[rand.Intn(len(truths))]))
	} else {
		ctx.SendChain(message.Text("大冒险: ", dares[rand.Intn(len(dares))]))
	}
}
